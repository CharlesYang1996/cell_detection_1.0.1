...
for m in range(10,11):#对某些点进行测试
    x_sample = dict_[m][0]
    y_sample = dict_[m][1]
    angle_temp_list = angle_round(x_sample, y_sample, 60)  # 第三个参数为圆的半径
    ...
    a = distance_from_single_point_to_center_list.copy()
    mark1 = 0#初始化
    mark2 = 0#初始化
    for i in range(1,len(a)-2):
        ...
        if abs(fall)>7:
            if mark1==0:
                ...
                #print("发现驼峰！")
                mark2=i+1
                wrong_numbers=mark2-mark1-1
                for k in range(1,wrong_numbers+1):
                    a[mark1+k]=a[mark1]+(1/(wrong_numbers+1))*k*(a[mark2]-a[mark1])
                    #print("成功修改")
                mark1=0
                mark2=0
        else:
    ...
    plt.plot([21.63]*len(a), color="red", linestyle='--')
    plt.plot([37.48 ] * len(a), color="red", linestyle='--')
